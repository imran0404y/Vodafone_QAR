...
public class Keyword_CRM extends Driver {
	...
	public String AddressCreation() {
		...
		try {
			...
			} else if (!(pulldata("Add_AddressLine1").equals(""))&&!(pulldata("Add_AddressLine1").equalsIgnoreCase("Autogenerated"))) {
			...
			} else if (!(pulldata("Add_AddressLine2").equals(""))&&!(pulldata("Add_AddressLine2").equalsIgnoreCase("Autogenerated"))) {
	...
	public String PlanSelection() {	
		...
		try {
			...
			String Reserve, Category, GetData, Field, Add_Addon, Remove_Addon, ReservationToken, SIM, MSISDN = null, SData = "SIM Card";
	...
	public String Entp_ContactCreation() {
		...
		try {
			...
			if (!(getdata("Cont_IDExpiryDate").equals(""))) {
				...
				Browser.WebTable.SetDataE("Acc_Contact", Row, Col, "VFQ_ID_Expiration_Date", pulldata("Cont_IDExpiryDate"));
			}
	...
	public String Plan_UpgradeDowngrade() {
		...
		String Test_OutPut = "", Status = "";
		String MSISDN, PlanName, PlanChange_From, GetData;
		...
		try {
			...
			if (!(getdata("New_PlanName").equals(""))) {
				PlanName = getdata("New_PlanName");
			} else {
				PlanName = pulldata("New_PlanName");
			}
			if (!(getdata("Existing_PlanName").equals(""))) {
				PlanChange_From = getdata("Existing_PlanName");
			} else {
				PlanChange_From = pulldata("Existing_PlanName");
			}
			if (!(getdata("GetData").equals(""))) {
				GetData = getdata("GetData");
			} else {
			...
			for (int i = 1; i <= Row_Count; i++) {
				...
				if (LData.equalsIgnoreCase(PlanChange_From)) {
